...
def build_patched_source(source: str) -> str:
    ...
    source = replace_once(
        ...
        '''video_only = temp / "video_only.mp4"\n        run([\n            "ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),\n            "-map", "0:v:0", "-c:v", "copy", "-an",\n            "-movflags", "+faststart", str(video_only),\n        ])\n\n        audio_sources = [opening_audio] + audios\n        if len(audio_sources) != len(clips):\n            raise RuntimeError(\n                f"Quantidade de áudios ({len(audio_sources)}) diferente dos clipes ({len(clips)})."\n            )\n\n        normalized_audio: List[Path] = []\n        for audio_index, (audio_source, clip_source) in enumerate(zip(audio_sources, clips)):\n            clip_duration = ffprobe_duration(clip_source)\n            normalized = temp / f"audio_pcm_{audio_index:03d}.wav"\n            run([\n                "ffmpeg", "-y", "-i", str(audio_source),\n                "-af", "apad", "-t", f"{clip_duration:.3f}",\n                "-ar", "44100", "-ac", "1", "-c:a", "pcm_s16le",\n                str(normalized),\n            ])\n            normalized_audio.append(normalized)\n\n        audio_concat = temp / "audio_concat.txt"\n        audio_concat.write_text(\n            "\\n".join(f"file '{item.as_posix()}'" for item in normalized_audio),\n            encoding="utf-8",\n        )\n        full_audio = temp / "audio_completo.wav"\n        run([\n            "ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(audio_concat),\n            "-c:a", "pcm_s16le", str(full_audio),\n        ])\n\n        run([\n            "ffmpeg", "-y", "-i", str(video_only), "-i", str(full_audio),\n            "-map", "0:v:0", "-map", "1:a:0",\n            "-c:v", "copy", "-c:a", "aac", "-b:a", "160k",\n            "-movflags", "+faststart", "-shortest", str(output_path),\n        ])''',
        "concatenação independente de vídeo e áudio PCM",
    )
    return source
